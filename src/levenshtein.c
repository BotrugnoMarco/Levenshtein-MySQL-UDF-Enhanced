...
int levenshtein(UDF_INIT *initid, UDF_ARGS *args, char *is_null, char *error) {
    ...
    if (args->arg_count == 3 && args->args[2]) {
        ...
        if (strcmp(flag, "0") == 0 || strcasecmp(flag, "false") == 0) {
            ignore_case = 0;
        }
    }
    ...
    if (ignore_case) {
        for (size_t i = 0; i < len1; i++) str1[i] = tolower(str1[i]);
        for (size_t i = 0; i < len2; i++) str2[i] = tolower(str2[i]);
    }
    ...
    for (size_t i = 0; i <= len1; i++) {
        ...
        if (!d[i]) {
            for (size_t k = 0; k < i; k++) free(d[k]);
            free(d); free(str1); free(str2);
            *error = 1;
            return -1;
        }
    }
